...
#print(wikipedia.page("List of companies listed on the Hong Kong Stock Exchange").section('0001 - 0099'))
...
count = 0
# Find top 30% performing stocks (relative to the HSI)
for ticker in tickers:
    count += 1
    
    # Download historical data as CSV for each stock (makes the process faster)
    df = pdr.get_data_yahoo(ticker, start_date, end_date)
    df.to_csv(f'../data/hk_stock/{ticker}.csv')
    ...
    print (f'Ticker: {ticker}; Returns Multiple against HSI: {returns_multiple} \t {count} / {list_length}\n')
    time.sleep(0.02)
...
for stock in rs_stocks:    
    try:
        df = pd.read_csv(f'../data/hk_stock/{stock}.csv', index_col=0)
        sma = [50, 150, 200]
        for x in sma:
